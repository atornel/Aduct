...
class Notebook(Gtk.Notebook):
    ...
    def add_child(self, child, position=-1):
        ...
        try:
            ...
            raise TypeError(
                "Quanta Notebook can only hold a child of type Quanta Element"
            )
    ...
    def change_child_label(self, child):
        ...
        (``Quanta.Element.child_name``).
        It is set to `No child` when `child` has no name for its child
        (``Quanta.Element.child_name is None``).
    ...
    def remove_child(self, child):
        ...
        if child in self.get_children():
            ...
            raise ValueError("Child not in Quanta Notebook")
